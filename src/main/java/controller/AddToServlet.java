    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (session == null || session.getAttribute("user_id") == null) {
            response.getWriter().write(
                    "{\"success\": false, \"message\": \"You are not logged in. Please login to add product.\", \"type\": \"error\"}");
            return;
        }
            ...
            switch (action) {
                case "/addToCart": {
                    ...
                    if (isAdded) {
                        ...
                        response.getWriter().write(
                                "{\"success\": true, \"message\": \"Product added to cart successfully!\", \"type\": \"success\", \"cart_count\": "
                                        + cartCount + "}");
                        // response.getWriter().write("{\"success\": true, \"message\": \"Product added
                        // to cart successfully!\", \"type\": \"success\"}");
                    } else {
                        response.getWriter().write(
                                "{\"success\": false, \"message\": \"Product is already in cart.\", \"type\": \"warning\"}");
                    }
                    break;
                }
                case "/addToWishlist": {
                    ...
                    if (isAdded) {
                        ...
                        response.getWriter().write(
                                "{\"success\": true, \"message\": \"Product added to wishlist successfully!\", \"type\": \"success\", \"wishlist_count\": "
                                        + wishlistCount + "}");
                    } else {
                        response.getWriter().write(
                                "{\"success\": false, \"message\": \"Product is already in wishlist.\", \"type\": \"warning\"}");
                    }
                    break;
                }
                case "/saveForLater": {
                    ...
                    if (isAdded && isRemoved) {
                        ...
                        response.getWriter()
                                .write("{\"success\": true, \"message\": \"Saved for later.\", \"type\": \"success\"}");
                    } else {
                        response.getWriter().write(
                                "{\"success\": false, \"message\": \"Failed to saved for later.\", \"type\": \"error\"}");
                    }
                    break;
                }
                case "/moveToCart": {
                    ...
                    if (isAdded && isRemoved) {
                        ...
                        session.setAttribute("checked_cart_count", checkedCartCount);
                        response.getWriter()
                                .write("{\"success\": true, \"message\": \"Moved to cart.\", \"type\": \"success\"}");
                    } else {
                        response.getWriter().write(
                                "{\"success\": false, \"message\": \"Failed to move to cart.\", \"type\": \"error\"}");
                    }
                    break;
                }
                default:
